...
void XWalkWebContentsDelegate::AddNewContents(content::WebContents* source, content::WebContents* new_contents,
  ...
  if (java_delegate.obj()) {
    create_popup = Java_XWalkWebContentsDelegate_addNewContents(env, java_delegate.obj(), is_dialog, user_gesture);
  }
...
void XWalkWebContentsDelegate::UpdatePreferredSize(content::WebContents* contents, const gfx::Size& pref_size) {
  ...
  ScopedJavaLocalRef<jobject> java_delegate = GetJavaDelegate(env);
  if (java_delegate.obj()) {
    Java_XWalkWebContentsDelegate_updatePreferredSize(env, java_delegate.obj(), pref_size.width(), pref_size.height());
  }
}
...
void XWalkWebContentsDelegate::RunFileChooser(content::RenderFrameHost* render_frame_host,
  ...
  Java_XWalkWebContentsDelegate_shouldOverrideRunFileChooser(
      env, java_delegate.obj(), render_frame_host->GetProcess()->GetID(), render_frame_host->GetRoutingID(), mode,
      ConvertUTF16ToJavaString(env, base::JoinString(params.accept_types, base::ASCIIToUTF16(","))).obj(),
      params.capture);
}
...
bool XWalkWebContentsDelegate::DidAddMessageToConsole(content::WebContents* source, int32_t level,
  ...
  return Java_XWalkWebContentsDelegate_addMessageToConsole(env, GetJavaDelegate(env).obj(), jlevel, jmessage.obj(),
                                                           line_no, jsource_id.obj());
}

void XWalkWebContentsDelegate::HandleKeyboardEvent(content::WebContents* source,
...
bool XWalkWebContentsDelegate::ShouldCreateWebContents(content::WebContents* web_contents,
  ...
  if (obj.is_null())
    return true;
  ScopedJavaLocalRef<jstring> java_url = ConvertUTF8ToJavaString(env, target_url.spec());
  return Java_XWalkWebContentsDelegate_shouldCreateWebContents(env, obj.obj(), java_url.obj());
}
