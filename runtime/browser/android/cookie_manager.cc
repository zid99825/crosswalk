...
void CookieManager::RemoveSessionCookieAsyncHelper(base::WaitableEvent* completion) {
  ...
  GetCookieStore()->DeleteSessionCookiesAsync(
      base::Bind(&CookieManager::RemoveCookiesCompleted, base::Unretained(this), completion));
}
...
void CookieManager::RemoveCookiesCompleted(base::WaitableEvent* completion, int num_deleted) {
 ...
 *
 */
static ScopedJavaLocalRef<jbyteArray> SaveCookies(JNIEnv* env, const JavaParamRef<jobject>& jcaller) {
 ...
 *
 */
static jboolean RestoreCookies(JNIEnv* env, const JavaParamRef<jobject>& jcaller,
    ...
    if (it.ReadInt(&cnt) && cnt > 0) {
      ...
      for (int i = 0; i < cnt; ++i) {
        ...
        if (/*it.ReadString(&spec) && */it.ReadString(&name) && it.ReadString(&value) && it.ReadString(&domain)
          ...
          // TODO READ url!!!
          cs->SetCookieWithDetailsAsync(url, name, value, domain, path, base::Time::FromInternalValue(c_time),
                                        base::Time::FromInternalValue(e_time), base::Time::FromInternalValue(l_time),
                                        secure, http_only, static_cast<net::CookieSameSite>(same_site),
...
static void SetCookie(JNIEnv* env, const JavaParamRef<jobject>& obj, const JavaParamRef<jstring>& url,
...
static ScopedJavaLocalRef<jstring> GetCookie(JNIEnv* env, const JavaParamRef<jobject>& obj,
...
static jboolean AllowFileSchemeCookies(JNIEnv* env, const JavaParamRef<jobject>& obj) {
  return CookieManager::GetInstance()->AllowFileSchemeCookies();
}

static void SetAcceptFileSchemeCookies(JNIEnv* env, const JavaParamRef<jobject>& obj, jboolean accept) {
  return CookieManager::GetInstance()->SetAcceptFileSchemeCookies(accept);
}

static void SetDbKey(JNIEnv* env, const JavaParamRef<jobject>& obj, const JavaParamRef<jstring>& dbKey) {
  std::string db_key(ConvertJavaStringToUTF8(env, dbKey));

  CookieManager::GetInstance()->SetDbKey(db_key);
}

static jint RekeyDb(JNIEnv* env, const JavaParamRef<_jobject*>& obj, const JavaParamRef<_jstring*>& oldKey,
...
static void SetZone(JNIEnv* env, const JavaParamRef<jobject>& obj, const JavaParamRef<jstring>& zone) {
  std::string zone_str(ConvertJavaStringToUTF8(env, zone));

  CookieManager::GetInstance()->SetZone(zone_str);
}

static jint NukeDomain(JNIEnv* env, const JavaParamRef<jobject>& obj, const JavaParamRef<jstring>& domain) {
  std::string domain_str(ConvertJavaStringToUTF8(env, domain));

  return CookieManager::GetInstance()->NukeDomain(domain_str);
}

static void PageLoadStarted(JNIEnv* env, const JavaParamRef<jobject>& obj, const JavaParamRef<jstring>& url) {
