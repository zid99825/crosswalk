/*
 * xwalk_proxying_url_loader_factory.cc
 *
...
class InterceptedRequest : public network::mojom::URLLoader,
 ...
 public:
  InterceptedRequest(int process_id, uint64_t request_id, int32_t routing_id, uint32_t options,
                     ...
                     network::mojom::URLLoaderFactoryPtr target_factory, bool intercept_only);
  ...
  void ContinueAfterInterceptWithOverride(std::unique_ptr<XWalkWebResourceResponse> response);

  void InterceptResponseReceived(std::unique_ptr<XWalkWebResourceResponse> response);

  // Returns true if the request was restarted or completed.
  bool InputStreamFailed(bool restart_needed);

 private:
  std::unique_ptr<XWalkContentsIoThreadClient> GetIoThreadClient();
...
void InterceptedRequest::Restart() {
  std::unique_ptr<XWalkContentsIoThreadClient> io_thread_client = GetIoThreadClient();
  ...
  request_.load_flags = UpdateLoadFlags(request_.load_flags, io_thread_client.get());
  if (!io_thread_client || ShouldNotInterceptRequest()) {
    ...
    io_thread_client->ShouldInterceptRequestAsync(
        XWalkWebResourceRequest(request_),
        base::BindOnce(&InterceptedRequest::InterceptResponseReceived, weak_factory_.GetWeakPtr()));
  }
}
...
void InterceptedRequest::InterceptResponseReceived(std::unique_ptr<XWalkWebResourceResponse> response) {
...
//  // OOR-CORS is enabled.
//  if (!request_.headers.HasHeader(content::kCorsExemptRequestedWithHeaderName)) {
//    request_.cors_exempt_headers.SetHeader(content::kCorsExemptRequestedWithHeaderName,
...
//  if (!client) {
//    DLOG(WARNING) << "client is null, onReceivedHttpError dropped for " << request.url;
//    return;
//  }
//  client->OnReceivedHttpError(request, std::move(http_error_info));
//}

//void OnReceivedErrorOnUiThread(int process_id, int render_frame_id, const XWalkWebResourceRequest& request, int error_code,
//                               bool safebrowsing_hit) {
//  auto* client = GetXWalkContentsClientBridgeFromID(process_id, render_frame_id);
//  if (!client) {
//    DLOG(WARNING) << "client is null, onReceivedError dropped for " << request.url;
//    return;
//  }
...
//    // status code >= 400.
//    std::unique_ptr<XWalkContentsClientBridge::HttpErrorInfo> error_info = XWalkContentsClientBridge::ExtractHttpErrorInfo(
//        head.headers.get());
//
//    base::PostTaskWithTraits(
//    FROM_HERE, {content::BrowserThread::UI},
//    base::BindOnce(&OnReceivedHttpErrorOnUiThread, process_id_,
//        request_.render_frame_id, XWalkWebResourceRequest(request_),
//        std::move(error_info)));
//  }
//
...
void InterceptedRequest::OnReceiveRedirect(const net::RedirectInfo& redirect_info,
...
void InterceptedRequest::OnUploadProgress(int64_t current_position, int64_t total_size,
...
std::unique_ptr<XWalkContentsIoThreadClient> InterceptedRequest::GetIoThreadClient() {
...
//  }
  ...
  return
      process_id_ ?
          XWalkContentsIoThreadClient::FromID(process_id_, request_.render_frame_id) :
          XWalkContentsIoThreadClient::FromID(request_.render_frame_id);
}
...
//============================
// XWalkProxyingURLLoaderFactory
//============================

XWalkProxyingURLLoaderFactory::XWalkProxyingURLLoaderFactory(
    int process_id, network::mojom::URLLoaderFactoryRequest loader_request,
  ...
  if (target_factory_info) {
    ...
    target_factory_.set_connection_error_handler(
        base::BindOnce(&XWalkProxyingURLLoaderFactory::OnTargetFactoryError, base::Unretained(this)));
  }
  proxy_bindings_.AddBinding(this, std::move(loader_request));
  proxy_bindings_.set_connection_error_handler(
      base::BindRepeating(&XWalkProxyingURLLoaderFactory::OnProxyBindingError, base::Unretained(this)));
}
...
// static
void XWalkProxyingURLLoaderFactory::CreateProxy(int process_id,
                                                network::mojom::URLLoaderFactoryRequest loader_request,
                                                network::mojom::URLLoaderFactoryPtrInfo target_factory_info) {
  DCHECK_CURRENTLY_ON(content::BrowserThread::IO);

  // will manage its own lifetime
  new XWalkProxyingURLLoaderFactory(process_id, std::move(loader_request), std::move(target_factory_info), false);
}

void XWalkProxyingURLLoaderFactory::CreateLoaderAndStart(
    network::mojom::URLLoaderRequest loader, int32_t routing_id, int32_t request_id, uint32_t options,
    const network::ResourceRequest& request, network::mojom::URLLoaderClientPtr client,
    const net::MutableNetworkTrafficAnnotationTag& traffic_annotation) {
  ...
  bool global_cookie_policy = XWalkCookieAccessPolicy::GetInstance()->GetShouldAcceptCookies();
  // process_id == 0 means the render_frame_id is actually a valid
  // frame_tree_node_id, otherwise use it as a valid render_frame_id.
  int frame_tree_node_id = process_id_ ? content::RenderFrameHost::kNoFrameTreeNodeId : request.render_frame_id;
  bool third_party_cookie_policy = XWalkCookieAccessPolicy::GetInstance()->GetShouldAcceptThirdPartyCookies(
  ...
  InterceptedRequest* req = new InterceptedRequest(process_id_, request_id, routing_id, options, request,
                                                   traffic_annotation, std::move(loader), std::move(client),
                                                   std::move(target_factory_clone), intercept_only_);
  req->Restart();
}

void XWalkProxyingURLLoaderFactory::OnTargetFactoryError() {
  delete this;
}

void XWalkProxyingURLLoaderFactory::OnProxyBindingError() {
  if (proxy_bindings_.empty())
    delete this;
}

void XWalkProxyingURLLoaderFactory::Clone(network::mojom::URLLoaderFactoryRequest loader_request) {
  DCHECK_CURRENTLY_ON(content::BrowserThread::IO);
  proxy_bindings_.AddBinding(this, std::move(loader_request));
}
