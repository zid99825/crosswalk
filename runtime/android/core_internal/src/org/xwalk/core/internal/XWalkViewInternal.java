...
public class XWalkViewInternal extends android.widget.FrameLayout {
   ...
   */
  @XWalkAPI(
      preWrapperLines = {"        super(${param1}, null);",
          "        SurfaceView surfaceView = new SurfaceView(${param1});",
          "        surfaceView.setLayoutParams(new ViewGroup.LayoutParams(0, 0));",
          "        addView(surfaceView);"},
      postWrapperLines = {"        addView((FrameLayout)bridge, new FrameLayout.LayoutParams(",
   ...
   */
  @XWalkAPI(
      preWrapperLines = {"        super(${param1}, ${param2});",
      ...
      postWrapperLines = {"        addView((FrameLayout)bridge, new FrameLayout.LayoutParams(",
          ...
          "                FrameLayout.LayoutParams.MATCH_PARENT));", "        removeViewAt(0);",
          "        new org.xwalk.core.extension.XWalkExternalExtensionManagerImpl(this);"},
      postBridgeLines = {"        String animatable = null;", "        try {",
  ...
  /**
   * Load a web page/app from a given base URL or a content. If url is null or empty and content is
   * null or empty, then this function will do nothing. If content is not null, load the web
   * page/app from the content. If content is not null and the url is not set, return "about:blank"
   * if calling {@link XWalkViewInternal#getUrl()}. If content is null, try to load the content from
   * the url.
   *
   * It supports URL schemes like 'http:', 'https:' and 'file:'. It can also load files from Android
   * assets, e.g. 'file:///android_asset/'.
   * 
   ...
   *             {@link XWalkViewInternal#loadData} or {@link XWalkViewInternal#loadDataWithBaseURL}
   *             instead.
   * @since 1.0
   */
  @Deprecated
   ...
   *
   * Note that JavaScript's same origin policy means that script running in a page loaded using this
   * method will be unable to access content loaded using any scheme other than 'data', including
   * 'http(s)'. To avoid this restriction, use {@link XWalkViewInternal#loadDataWithBaseURL} with an
   ...
   * octets outside that range. For example, '#', '%', '\', '?' should be replaced by %23, %25, %27,
   * %3f respectively.
   *
   * The 'data' scheme URL formed by this method uses the default US-ASCII charset. If you need need
   * to set a different charset, you should form a 'data' scheme URL which explicitly specifies a
   * charset parameter in the mediatype portion of the URL and call
   ...
   * policy. The historyUrl is used for the history entry.
   *
   * Note that content specified in this way can access local device files (via 'file' scheme URLs)
   * only if baseUrl specifies a scheme other than 'http', 'https', 'ftp', 'ftps', 'about' or
   * 'javascript'.
   *
   * If the base URL uses the data scheme, this method is equivalent to calling
   * {@link XWalkViewInternal#loadData} and the historyUrl is ignored, and the data will be treated
   * as part of a data: URL. If the base URL uses any other scheme, then the data will be loaded
   * into the WebView as a plain string (i.e. not part of a data URL) and any URL-encoded entities
   * in the string will not be decoded.
   *
   * Note that the baseUrl is sent in the 'Referer' HTTP header when requesting subresources
   * (images, etc.) of the page loaded using this method.
   *
   ...
   *        URL, specified as a map from name to value. Note that if this map contains any of the
   *        headers that are set by default by this XWalkView, such as those controlling caching,
   *        accept types or the User-Agent, their values may be overriden by this XWalkView's
   *        defaults.
   * @since 8.0
   */
  @XWalkAPI
  ...
  /**
   * Load a web app from a given manifest.json file. If content is not null, load the manifest.json
   * from the content. If content is null, try to load the manifest.json from the url. Note that url
   * should not be null if the launched path defined in manifest.json is relative.
   *
   * It supports URL schemes like 'http:', 'https:' and 'file:'. It can also load files from Android
   * assets, e.g. 'file:///android_asset/'.
   * 
  ...
  /**
   * Get the content height of current web page/app. NOTE: When page finished loading, it's content
   * size info may not be updated. Pls wait for a while for info updating.
   * 
  ...
  /**
   * Injects the supplied Java object into this XWalkViewInternal. Each method defined in the class
   * of the object should be marked with {@link JavascriptInterface} if it's called by JavaScript.
   * 
  ...
  /**
   * Clear the resource cache. Note that the cache is per-application, so this will clear the cache
   * for all XWalkViews used.
   * 
  ...
  /**
   * Pause all layout, parsing and JavaScript timers for all XWalkViewInternal instances. It will be
   * called when the container Activity get paused. It can also be explicitly called to pause
   * timers.
   *
   * Note that it will globally impact all XWalkViewInternal instances, not limited to just this
   * XWalkViewInternal.
   *
   * @since 1.0
   */
   ...
   * modal dialogs, etc. See {@link #pauseTimers} about pausing JavaScript timers. It will be called
   * when the container Activity get paused. It can also be explicitly called to pause above things.
   * 
   * @since 1.0
   */
  @XWalkAPI
  ...
  /**
   * Pass through activity result to XWalkViewInternal. Many internal facilities need this to handle
   * activity result like JavaScript dialog, Crosswalk extensions, etc. See
   ...
   * 
   * @param factor the zoom factor to apply. The zoom factor will be clamped to the XWalkView's zoom
   *        limits. This value must be in the range 0.01 to 100.0 inclusive.
   * @since 5.0
   */
  @XWalkAPI
  ...
  private static void checkThreadSafety() {
    if (Looper.myLooper() != Looper.getMainLooper()) {
      Throwable throwable =
          new Throwable("Warning: A XWalkViewInternal method was called on thread '"
   ...
   * only affects the display of the autocomplete popup, it does not remove any saved form data from
   * this WebView's store. This is a poorly named method, but we keep it for historical reasons.
   * 
   * @since 6.0
   */
  @XWalkAPI
  ...
  @XWalkAPI(disableReflectMethod = true,
      preWrapperLines = {"        if (visibility == View.INVISIBLE) visibility = View.GONE;",
