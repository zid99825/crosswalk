...
class XWalkDevToolsServer {
    ...
    private static boolean checkDebugPermission(int pid, int uid) {
        String debugPermissionName = ContextUtils.getApplicationContext().getPackageName()
                + DEBUG_PERMISSION_SIFFIX;
        return ContextUtils.getApplicationContext().checkPermission(debugPermissionName, pid,
