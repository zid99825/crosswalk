...
JsJavaConfigurator::JsJavaConfigurator(content::RenderFrame* render_frame)
  ...
  render_frame->GetAssociatedInterfaceRegistry()->AddInterface(
      base::BindRepeating(&JsJavaConfigurator::BindPendingReceiver, base::Unretained(this)));
}

JsJavaConfigurator::~JsJavaConfigurator() = default;

void JsJavaConfigurator::SetJsApiService(bool need_to_inject_js_object, const std::string& js_object_name,
...
inline bool JsJavaConfigurator::IsOriginMatch() {
  url::Origin frame_origin = url::Origin(render_frame()->GetWebFrame()->GetSecurityOrigin());
  return js_object_allowed_origin_rules_.Matches(frame_origin.GetURL());
}

void JsJavaConfigurator::BindPendingReceiver(
    mojo::PendingAssociatedReceiver<mojom::JsJavaConfigurator> pending_receiver) {
  LOG(INFO) << "iotto " << __func__;
  receiver_.Bind(std::move(pending_receiver), render_frame()->GetTaskRunner(blink::TaskType::kInternalIPC));
}
